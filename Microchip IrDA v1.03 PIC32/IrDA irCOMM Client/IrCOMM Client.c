...
Microchip IrDA Standard Stack - irCOMM 3-wire Raw Client Demo

irCOMM Client.c

This demo program shows how to use the irCOMM 3-wire raw protocol in client
mode.
...
This program is designed to work with the IrDA irCOMM Server Demo.  Follow
...
int main( void )
{
    ...
    result = IrDA_OpenCommClient( COMM_THREE_WIRE_RAW );
    if (result)
    {
        UARTxPutHex( result );
    ...
    else
    {
        for (j=0; j<TEST_STRING_TRANSMISSIONS; j++)
        {
            ...
            result = IrDA_WriteComm( myData, sizeof(myData)-1 );
            if (result)
            {
                UARTxPutHex( result );
        ...
        result = IrDA_CloseCommClient( 2000 );
        if (result)
        {
            UARTxPutHex( result );
